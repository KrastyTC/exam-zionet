...
STRING_AGG(ISNULL(par.first_name+par.last_name+'('+par.number+')', 'Orphan'),',') as parents FROM student
LEFT JOIN parent_to_student as pts on pts.student_id = student_id
LEFT JOIN parent as par on par.id = pts.parent_id
GROUP BY Student
